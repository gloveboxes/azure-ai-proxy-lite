...
```
┌────────────┐        api-key (user)       ┌─────────────────┐      api-key (backend)      ┌────────────────┐
│  MCP Client│ ──────────────────────────▸  │  Azure AI Proxy │ ──────────────────────────▸  │  MCP Server    │
│  (VS Code, │  POST /api/v1/mcp/{name}/…  │                 │  POST {endpointUrl}/…        │  (FastMCP etc) │
│  Claude,   │ ◂────────────────────────── │  authenticates  │ ◂────────────────────────── │                │
│  Python)   │        streamed response     │  & forwards     │        streamed response     │                │
└────────────┘                              └─────────────────┘                              └────────────────┘
```
...
3. **Backend MCP server** receives the request with the backend `api-key` (if configured) and returns a response.
...
### API key on the MCP server

The proxy forwards the catalog's **Endpoint Key** to the backend as an `api-key` header. The MCP server should validate this header if a key is configured. The `ApiKeyMiddleware` above does exactly this:

- Set `MCP_API_KEY` on the server to a secret value.
- Enter that same value as the **Key** field when registering the catalog in the proxy admin UI.
- The proxy encrypts and stores it, then sends it on every forwarded request.

If you leave both the server's `MCP_API_KEY` and the catalog **Key** empty, no backend auth is enforced (useful for local development).
...
> **Important**: The **Deployment Name** you set here is what clients use in the URL path: `/api/v1/mcp/{deploymentName}/...`
...
There are **two separate API keys** involved:
...
A complete working example is in [`examples/python/openai_sdk_1.x/mcp/`](examples/python/openai_sdk_1.x/mcp/):
...
See the [example README](examples/python/openai_sdk_1.x/mcp/README.md) for local and Azure deployment instructions.
